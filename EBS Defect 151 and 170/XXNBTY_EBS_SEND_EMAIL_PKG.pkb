...
IS
  ...
  -- 29-Apr-2015: End of line for defect #170 and INC960237.  
    ...
    WHEN 'BOM' THEN
      ...
      v_rec.where_clause     := 'bici.process_flag = 3 AND TRUNC(bici.creation_date) = TRUNC(SYSDATE) AND bici.transaction_id = mie.transaction_id AND abs(bici.request_id) >= '||v_main_request_id;  --- 23-Apr-2015: Added the "AND TRUNC(creation_date) = TRUNC(SYSDATE)" to get the current error and error message to resolved defect 170.
      ...
      v_rec.table_type       := 'INTERFACE';
      v_rec.where_clause     := 'bbmi.process_flag = 3 AND TRUNC(bbmi.creation_date) = TRUNC(SYSDATE) AND bbmi.transaction_id = mie.transaction_id AND abs(bbmi.request_id) >= '||v_main_request_id; --- 23-Apr-2015: Added the "AND TRUNC(creation_date) = TRUNC(SYSDATE)" to get the current error and error message to resolved defect 170.
      v_rec.group_by_clause  := 'mie.error_message';
      v_query(4)             := v_rec;
    WHEN 'FORMULA' THEN 
      v_rec.column_name      := 'error_text, count(*)';
    ...
    WHEN 'CUSTOMER_INTERFACE' THEN
      ...
      v_rec.table_type       := 'INTERFACE';
      v_rec.where_clause     := 'interface_status IS NOT NULL AND TRUNC(creation_date) = TRUNC(SYSDATE)';
